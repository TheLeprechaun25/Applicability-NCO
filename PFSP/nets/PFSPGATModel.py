...
class PFSPModel(nn.Module):
    ...
    def forward(self, instance, edges, decode_type='greedy'):
        batch_size, n_jobs, n_machines = instance.shape
        ...
        U = np.arange(0, n_jobs, dtype=int)
        ...
        for step in range(n_jobs):
            ...
            nodes = compute_nodes(batch_size, n_jobs, n_machines, instance, np_nodes, S, U, step, device=instance.device)
            ...
            k = k.reshape(batch_size, n_jobs, self.n_heads, -1).transpose(1, 2)
            v = v.reshape(batch_size, n_jobs, self.n_heads, -1).transpose(1, 2)
...
class MHALayer(nn.Module):
    def __init__(self, **model_params):
        ...
        """
        1) e_ij = (W_q(h_i) * W_k(h_j) * W_e(E_ij)) / d_k^2 <- Multiplicative Attention
        2) e_ij = a * LeakyReLU(W_e * E_ij + W_l * h_i + W_r * h_j) <- Additive Attention
        """
        self.n_heads = model_params['n_heads']
