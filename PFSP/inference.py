...
tester_params = {
    ...
    'taillard': False,
    # Initialization
    'folder_path': f'results/used_models/{model_size}',  # directory path of pre-trained model and log files saved.
...
def main():
    if tester_params['model_size'] == 'small':
        ...
        raise ValueError(f"Invalid model size: {tester_params['model_size']}")
    ...
    if tester_params['save_results']:
        ...
        with open(f"PFSP/results/inference_results/NCO_{model_size}_{size}_scores.pkl", "wb") as f:
